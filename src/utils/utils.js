...
export const changModel = (model, nodeName, options) => {
  if (!model || !model.materials || !model.materials[nodeName]) {
    console.warn(`Material ${nodeName} not found in model`);
    return;
  }
  try {
    model.materials[nodeName].setValues(options);
  } catch (error) {
    console.warn(`Error setting values for material ${nodeName}:`, error);
  }
};
